    ...
    : InteractivePlugin<MemoryService>, IConfigurable<MemoryConfig>
{
    ...
    public override async Task AwakeAsync(AwakeContext context)
    {
        ...
        Prompt("""
               ...
               ## 压缩规则要点说明
               注意！描述事情时，你要遵守如下规则：
               1. 无需记录存档信息，仅直接总结压缩时提供的内容中的事件即可。
               2. 不要将被压缩内容之外其他聊天记录混淆其中，不要记录重复的内容。
               3. 压缩时注意保留其中的重要经历、关键性信息等，以形成永久的核心记忆。
               4. 当要压缩内容过多时，要按重要程度进行取舍，如核心记忆需优先保留，重复、突发性琐事可以简略。
               5. 在压缩总结的写法上，对于多事件内容要按时间段区分，对于核心记忆则要单独记录。
               6. 分清事件中的具体人物，不要用‘你’这种代词，要用具体的名称，如‘主人’、‘某某某’等。
               7. 系统会自动生成存档信息，所以不要擅自添加系统信息，直接像讲故事一样描述概述内容中发生的事件即可。
               """);
    }
